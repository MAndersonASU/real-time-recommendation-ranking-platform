...
- **CPU vs. wall time** — `time.process_time()` alongside
 `time.perf_counter()` over real requests. Their divergence reveals
  something `docs/experiments/serving-latency.md`'s wall-clock-only numbers couldn't: whether a
...
(`docs/experiments/optimization.md`) is scoped to act on, not this document, which is
