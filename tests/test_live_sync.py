...
# script's contract, which the idempotency guarantee depends on. Real
# latency and expiry behaviour are proven separately against the running
# container, in verify_state_store.py and verify_live_sync.py.
...
def test_syncing_consumer_writes_recent_features_after_every_event():
    ...
    assert features.last_event_time == "2019-11-14T09:00:00"
...
def test_plain_stream_consumer_state_updated_hook_is_a_no_op():
    ...
    processed = consumer.process(make_event(EventType.CLICK, "u1", "n1", 1, "2019-11-14T08:00:00").to_json())
...
def test_syncing_consumer_restores_prior_state_after_a_restart():
    ...
    consumer_before_restart.process(make_event(EventType.IMPRESSION, "u1", "n1", 1, "2019-11-14T08:00:00").to_json())
    consumer_before_restart.process(make_event(EventType.CLICK, "u1", "n1", 1, "2019-11-14T08:00:00").to_json())
    ...
    consumer_after_restart.process(make_event(EventType.IMPRESSION, "u1", "n2", 2, "2019-11-14T09:00:00").to_json())
    ...
    assert after.last_event_time == "2019-11-14T09:00:00"
...
def test_syncing_consumer_does_not_double_count_after_a_crash_before_commit():
    ...
    """
    client = _FakeRedis()
    click_event = make_event(EventType.CLICK, "u1", "n1", 1, "2019-11-14T08:00:00")
...
def test_syncing_consumer_restores_correct_state_for_the_user_after_a_redelivery():
    ...
    """
    client = _FakeRedis()
    first = make_event(EventType.CLICK, "u1", "n1", 1, "2019-11-14T08:00:00").to_json()
    ...
    restarted.process(make_event(EventType.CLICK, "u1", "n2", 1, "2019-11-14T09:00:00").to_json())
...
def test_two_consumers_that_both_read_before_writing_lose_nothing():
    ...
    first.process(make_event(EventType.CLICK, "u1", "n1", 1, "2019-11-14T08:00:00").to_json())
    second.process(make_event(EventType.CLICK, "u1", "n2", 2, "2019-11-14T09:00:00").to_json())
...
def test_an_impression_is_never_reapplied_as_a_click():
    ...
    SyncingStreamConsumer(client).process(
        make_event(EventType.CLICK, "u1", "n1", 1, "2019-11-14T08:00:00").to_json()
    )

    SyncingStreamConsumer(client).process(
        make_event(EventType.IMPRESSION, "u1", "n9", 2, "2019-11-14T09:00:00").to_json()
    )
...
def test_impression_and_click_conflicts_each_apply_exactly_once():
    ...
    for first_type, second_type in (
        ...
        a.process(make_event(first_type, "u1", "n1", 1, "2019-11-14T08:00:00").to_json())
        b.process(make_event(second_type, "u1", "n2", 2, "2019-11-14T09:00:00").to_json())
...
def test_a_late_duplicate_does_not_roll_state_back():
    ...
    """
    client = _FakeRedis()
    event_a = make_event(EventType.CLICK, "u1", "n1", 1, "2019-11-14T08:00:00").to_json()
    event_b = make_event(EventType.CLICK, "u1", "n2", 2, "2019-11-14T09:00:00").to_json()
...
def test_an_expired_claim_lets_an_event_apply_again():
    ...
    client = _FakeRedis()
    event = make_event(EventType.CLICK, "u1", "n1", 1, "2019-11-14T08:00:00").to_json()
