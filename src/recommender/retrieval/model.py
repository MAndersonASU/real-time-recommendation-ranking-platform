...
class ItemTower(nn.Module):
    """Category + subcategory -> item vector. Works for any catalog item,
    seen or unseen during training, since these features never depend on
    click history.
    """

    def __init__(self, num_categories: int, num_subcategories: int, embedding_dim: int = 32):
        super().__init__()
    ...
    def forward(self, category_idx: torch.Tensor, subcategory_idx: torch.Tensor) -> torch.Tensor:
        x = torch.cat([self.category_emb(category_idx), self.subcategory_emb(subcategory_idx)], dim=-1)
        return self.proj(x)
...
class TwoTowerModel(nn.Module):
    ...
    def __init__(self, num_categories: int, num_subcategories: int, embedding_dim: int = 32):
        super().__init__()
        self.item_tower = ItemTower(num_categories, num_subcategories, embedding_dim)
    ...
    def user_vector(
        ...
        item_vecs = self.item_tower(history_category_idx, history_subcategory_idx)
        mask = history_mask.unsqueeze(-1)
        summed = (item_vecs * mask).sum(dim=1)
        counts = mask.sum(dim=1).clamp(min=1.0)
        return summed / counts

    def item_vector(self, category_idx: torch.Tensor, subcategory_idx: torch.Tensor) -> torch.Tensor:
        return self.item_tower(category_idx, subcategory_idx)
    ...
    def forward(
        ...
        u = self.user_vector(history_category_idx, history_subcategory_idx, history_mask)
        v = self.item_vector(cand_category_idx, cand_subcategory_idx)
